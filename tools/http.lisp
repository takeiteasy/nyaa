...
(defun perform-request (url method headers body timeout-ms)
  ...
  ;; ATTEMPT-REQUEST opens it, so a lapsed deadline has something to close.
  (let ((result nil)
        (socket-box (list nil))
    ...
    (bt:make-thread
     (lambda ()
       (unwind-protect
            (setf result (attempt-request url method headers body socket-box
                                          timeout-ms))
    ...
    (if (bt:wait-on-semaphore done :timeout (/ timeout-ms 1000))
        result
        (progn
          ;; Closing the socket unblocks the worker thread wherever the
          ;; exchange stalled -- connecting, writing, or waiting on a
          ;; response -- so it errors out and unwinds there instead of
          ;; running until the server answers or the connection drops.
          ;;
          ;; The close itself runs in its own thread rather than PERFORM-
          ;; REQUEST's: on SBCL it returns promptly, but on ECL closing a
          ;; socket another thread is blocked reading from does not
          ;; interrupt that read -- the close call blocks right alongside
          ;; it until the peer resolves the connection. Backgrounding it
          ;; keeps this deadline honoured either way; the worker thread's
          ;; own lifetime past the deadline is what ~takeiteasy/nyaa#55
          ;; tracks on ECL specifically.
